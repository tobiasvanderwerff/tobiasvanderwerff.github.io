...
In the last year or so, I've become quite interested in low-level GPU programming for performance optimization of neural nets. 
Diving into CUDA programming has been a fun (and challenging!) way to explore the magic behind what makes neural networks run fast on GPUs.[^1]
...
If there's one thing I've noticed, it's that learning to write high-performance CUDA code is like learning to program in hard mode. In my experience, one does not simply "learn" CUDA -- one merely becomes less incompetent over time.
<!--it's more like getting sucked into the abyss and going through lots of pain and suffering before you can go hope to go from being completely hopeless at it to perhaps mildly competent.-->
<!-- in my experience, you inevitably make so many mistakes that -->
<!-- the best you can hope for is to make sufficient number of mistakes such that at some point, you can avoid common pitfalls/go from being "completely hopeless" to "mildly competent".-->
<!--Another lesson I've learned is the importance of _actually reading the manual_.-->
The devil tends to be in the details when you write CUDA code, and the details are far too easy to miss if you're not careful.

In this post, I want to highlight a specific detail of CUDA C/C++ that can easily lead to mistakes: the `__CUDA_ARCH__` macro. It's caused me a fair bit of confusion in the last few days, so this post acts as an overview of my findings on how to use `__CUDA_ARCH__` while avoiding some of the pitfalls.
...
Inside device code (CUDA code), the `__CUDA_ARCH__` macro expands to the compute capability of the GPU that you're compiling for. For example, on an NVIDIA A100, which has compute capability sm80, `__CUDA_ARCH__` will expand to `800`. For a GeForce RTX 4090, which has compute capability sm89, `__CUDA_ARCH__` will expand to `890`. By contrast, inside host code (CPU code), the value of the macro is undefined. We can use the value of `__CUDA_ARCH__` to conditionally include code that may only work for specific GPU architectures.
...
Although the `__CUDA_ARCH__` macro is quite useful, it needs to be used with care. In particular, there are various situations where using it actually leads to [undefined behavior](https://en.wikipedia.org/wiki/Undefined_behavior). The CUDA C++ programming guide has a [section](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#cuda-arch) that indicates the four situations where using `__CUDA_ARCH__` leads to undefined behavior[^2]. These are[^3]:
...
What exacty happens when you write code that violates any of these four rules? The docs provide the answer:
...
The idea here is that the FP6 kernel only supports sm80 and higher, so we use `__CUDA_ARCH__` to only include the code when `__CUDA_ARCH__ >= 800` or when `__CUDA_ARCH__` is undefined (which is the case for host code). This avoids compilation problems when using GPUs with compute capability older than sm80.
...
If we refer back to the four rules I mentioned earlier from the [CUDA C++ Programming Guide](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#cuda-arch), it appears that we are violating rule no. 2. Here it is in the docs:
...
This means there is a problematic divergence between the host and device code: The host code includes a `__global__` function symbol that the device code does not. What happens as a result is undefined behavior, i.e. anything can happen[^4]. This explained the inconsistent behavior of the kernel, which went away after addressing this issue.
